				...
				<table class="table table-condensed">
					...
					<c:forEach items="${ticket }" var="t">
						<tr>
							...
							<td>${t.price }</td>
							<td>
								<c:if test="${t.status==1 }">售票中</c:if>
								<c:if test="${t.status==2 }"><span style="color: red;">已停售</span></c:if>
							</td>
							<td><input type="text" class="ck" id="num${t.tid }" style="width: 50px;" <c:if test="${t.status==2 }">disabled="disabled"</c:if>></td>
							<td><input type="button" class="btn" value="加入购物车" onclick="cart(${t.tid },${t.balance })"></td>
						</tr>
						</c:forEach>
					</tbody>
				</table>
...
<script type="text/javascript">
	...
	function cart(tid,balance) {
		...
		if(balance<=b){
			alert("没有用户需求的票数");
			location.href="<%=request.getContextPath()%>/user/ticket";
		}
		if(b==""){
			b=1
		}
